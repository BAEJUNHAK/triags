...
from utils.multiple_view_loss import compute_multi_view_losses, compute_tgpc_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # GT depth needs depth rendering from densify_from_iter, other losses need it from regularization_from_iter
        need_depth = (require_depth and reg_kick_on) or (use_gt_depth and iteration >= opt.densify_from_iter)
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, kernel_size, require_coord = require_coord and reg_kick_on, require_depth = need_depth, app_model=app_model)
        ...
        # GT depth supervision (median-aligned L1, densification phase only)
        gt_depth_loss = torch.tensor(0.0, device="cuda")
        if (use_gt_depth
            and iteration >= opt.densify_from_iter
            and iteration <= opt.densify_until_iter
            and viewpoint_cam.gt_depth is not None):
            rendered_depth = render_pkg["expected_depth"].squeeze(0)
            ...
            if num_valid > 100:
                ...
                # Per-image median alignment (detach to prevent scale gaming)
                scale = (torch.median(pred_valid) / torch.median(gt_valid)).detach()
                aligned_gt = gt_valid * scale

                gt_depth_loss = torch.nn.functional.l1_loss(pred_valid, aligned_gt)
        ...
        if iteration > opt.regularization_from_iter:
            ...
            if nearest_cam is not None:
                # Render nearest package for multi-view computations
                ...
                # Delegate multi-view losses (geo + ncc) and collect TGPC candidates
                mv_res = compute_multi_view_losses(viewpoint_cam=viewpoint_cam,
                                                  nearest_cam=nearest_cam,
                                                  scene=scene,
                                                  gaussians=gaussians,
                                                  render_pkg=render_pkg,
                                                  nearest_render_pkg=nearest_render_pkg,
                                                  gt_image_gray=gt_image_gray,
                                                  opt=opt,
                                                  use_virtul_cam=use_virtul_cam)

                geo_loss = mv_res['geo_loss']
                ncc_loss = mv_res['ncc_loss']
                final_valid_indices_for_tgpc = mv_res['tgpc_candidates']
                pts_flat = mv_res['pts']
                pixels_flat = mv_res['pixels']
                weights_flat = mv_res['weights']

                if geo_loss is not None:
                    loss += geo_loss

                tgpc_weight = opt.tgpc_loss_weight
                tgpc_num_neighbors = opt.tgpc_num_neighbors

                if tgpc_weight > 0.0 and final_valid_indices_for_tgpc.numel() > 0:
                    if iteration % 2500 == 0:
                        print("TGPC sampling num_valid_indices: ", final_valid_indices_for_tgpc.numel())
                    current_tgpc_loss = compute_tgpc_loss(final_valid_indices_for_tgpc,
                                                          pts_flat,
                                                          pixels_flat,
                                                          weights_flat,
                                                          viewpoint_cam,
                                                          ...
                                                          nearest_cam,
                                                          opt,
                                                          tgpc_num_neighbors=tgpc_num_neighbors)
                    if current_tgpc_loss is not None:
                        loss += tgpc_weight * current_tgpc_loss
                        tgpc_loss = current_tgpc_loss
